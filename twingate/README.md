...
1. [Create a new connector in Twingate](https://www.twingate.com/docs/deploy-connector-with-docker-compose) and copy the name, access token and refresh token. You will need them for the next steps.
2. Deploy first connector:
``` bash
mkdir ~/docker/containers/twingate-connector-1 -p
...
``` bash
mkdir ~/docker/containers/twingate-connector-2 -p
